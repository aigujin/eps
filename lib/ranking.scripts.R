...
# vvs.combine <- function(stocks, sectors.stock, broker, ret.vol, 
#     sector.ret,t) {
#     
#     stock.vvs <- abind(lapply(names(sectors.stock), function(sec) {
#         sec.s <- unique(intersect(stocks,sectors.stock[[sec]]))
#         load(analysts.dir(paste("/Sectors/", sec, "/raw.all.stock.vvs.RData", 
#             sep = "")))
#         sec.ret.array <- replicate(length(sec.s), sector.ret[, 
#             sec], simplify = "array")
#         abind(raw.all.stock.vvs[, sec.s, drop = F, ], sec.ret = aperm(sec.ret.array, 
#             c(1, 3, 2)), new.names = list(NULL, NULL, c(dimnames(raw.all.stock.vvs)[[3]], 
#             "sec.ret")))
#     }), along = 2)
#     comb.vvs <- stock.vvs[, unique(sort(dimnames(stock.vvs)[[2]])), ]
#     
#     period.vvs = c((dim(comb.vvs)[1] - t + 1):dim(comb.vvs)[1])
#     period.brok = c((dim(broker)[1] - t + 1):dim(broker)[1])
#   period.ret = c((dim(ret.vol)[2] - t + 1):dim(ret.vol)[2])
#     abind(broker[period.brok, stocks, ], comb.vvs[period.vvs, , ], stock.ret = t(ret.vol[stocks, period.ret]), along = 3)
# }
...
nominal.ranking.script.f <- function(methods, stock.names, stock.vvs,  tr, r.parameters,b) {
        n = as.numeric(r.parameters[1])
        diff.lag = as.numeric(r.parameters[2])
        sd.lag = as.numeric(r.parameters[3])
        x=as.numeric(r.parameters[4])
        ...
        abind(lapply(stock.names, function(s) {    
                ...
                mod.vvs <- acast(melt(setnames(data.table(melt(stock.vvs[,s,])),c('q.id','vvs','value'))[,state.vvs.f(value,diff.lag,sd.lag),by=vvs][,q.num:=1:.N,by=vvs],id.vars=c('vvs','q.num')),q.num~vvs~variable)
                
                abind(lapply(methods, function(meth) {
                nom.vvs <-removeNA(discretize(mod.vvs[,,meth],b))
                #discretize(mod.vvs[,,meth],b)
                        analyst.prediction.f(tr[, , s], nom.vvs, n)
                        #roll.analyst.prediction.f(tr[, , s], mod.vvs[,,meth], n,x)
                }),along=3)
        }),along = 4, new.names = list(dimnames(tr)[[2]], NULL, methods,stock.names))
} 

removeNA<-function(data)
{
        sapply(1:ncol(data), function (attr)
        {
                data[is.na(data[,attr]),attr] <- paste0("notApp",attr)
                data[,attr]
        })
}

discretize <- function(raw.data,n)
{
        sapply(1:ncol(raw.data), function (attr) 
        {
                
                if(!all(is.na(raw.data[,attr]))){
                        breaks = c(-Inf,quantile(raw.data[,attr], na.rm=T),Inf)
                        breaks = breaks + seq_along(breaks) * .Machine$double.eps
                        label <- paste0(letters[1:length(breaks)],attr)
                        as.character(cut(raw.data[,attr],breaks,labels=label))
                }else{rep(paste0("notApp",attr),nrow(raw.data))}
        })
}


analyst.prediction.f <- function(y,x,n){
        sapply(4:nrow(y),function(t){
                model <- nbrModel(x[1:(t-2),drop=F,],y[2:(t-1),drop=F,],round(timeWeights(t-2,n),4))
                predict.rank <- nbRank(model, y[2:(t-1),drop=F,],test=x[t-1,drop=F,])
