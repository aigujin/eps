...
eps.dt <- na.omit(setkey(eps.dt[,core.b:=.N>=12,by=list(Stock,Broker)][(core.b)][,clean.s:=.N>=3,by=list(q.id,Stock)][(clean.s)][,core.q:=length(unique(q.id))>=8,by=.(Stock)][(core.q)][,q.num:=.GRP,by=q.id],Stock)[eps.stocks])
...
complete.dt <- na.omit(setkey(complete.dt,Stock)[eps.stocks.rank][,eps.pmafe:=abs(Est-act.eps)/mean(abs(Est-act.eps))])
