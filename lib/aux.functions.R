...
cont.tab.f <- function(dt,t,n.b)
{
        ...
        require(descr)
        pt.cont.dt <- na.omit(rank.split)[,valid.s:=.N>1,by=.(Stock)][(valid.s)][,as.data.table(crosstab(cur.per,next.per,prop.r=T,prop.c=F,prop.t=F,prop.chisq = F,plot=F,missing.include=F,chisq=F)[2]),by=Stock][,value:=letters[1:9],by=Stock][,mean(prop.row),by=value]
