...
<<chp3-load-data,echo=F,warning=FALSE,message=FALSE>>=
...
#     ans
# }
# crossby(eps.dt,length(Stock),c('Sector'))

eps.stocks <- intersect(eps.dt$Stock,dimnames(array.all.vvs)[[2]])
eps.dt <- na.omit(setkey(eps.dt[,core.b:=.N>=12,by=list(Stock,Broker)][(core.b)][,clean.s:=.N>=3,by=list(q.id,Stock)][(clean.s)][,q.num:=.GRP,by=q.id],Stock)[eps.stocks])
...
complete.dt <- setnames(na.omit(setkey(complete.dt,Stock)[eps.stocks.rank][,eps.pmafe:=abs(Est-act.eps)/mean(abs(Est-act.eps))])[,q.id:=NULL],'fis.q','q.id')
...
stat.eps <- rbind(setnames(eps.dt[order(Sector),c(lapply(.SD,function(i){length(unique(i))}),.N),by=.(Sector),.SDcols=c('Stock','Broker')],4,'Forecast'),setnames(cbind('Total',eps.dt[order(Sector),c(lapply(.SD,function(i){length(unique(i))}),Forecast=.N),.SDcols=c('Stock','Broker')]),1,'Sector'))
...
num.f.dt <- eps.dt[order(Sector),.N,by=.(q.id,Sector,Broker,Stock)]

per.broker <- setnames(cbind(
        rbind(eps.dt[order(Sector),.N,by=.(q.id,Sector,Broker)][,mean(N),by=Sector],list('Total',eps.dt[order(Sector),.N,by=.(q.id,Sector,Broker)][,mean(N)])),
...
per.stock <- setnames(cbind(
        rbind(eps.dt[order(Sector),.N,by=.(q.id,Sector,Stock)][,mean(N),by=Sector],list('Total',eps.dt[order(Sector),.N,by=.(q.id,Sector,Stock)][,mean(N)])),
...
print(xtable(results.final,display=c('s','d','f','f','f')),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."))
@
\bottomrule
\end{tabularx}
\end{center}
...
\midrule
<<desc-sector,echo=F,results='asis'>>=
...
<<fig-mean-forecast,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Average number of forecast per quarter',fig.lp='mean-f'>>=
ggplot(num.f.dt[,mean(N),by=.(q.id,Sector)],aes(x=as.Date(q.id),y=V1))+geom_line()+facet_wrap(~Sector,ncol=5,scale='free_x')+theme_bw()+geom_smooth(method='loess')+xlab('Years')+ylab('Mean forecast')
@

<<fig-num-forecast,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Number of forecasts',fig.lp='num-f'>>=

ggplot(num.f.dt[,.N,by=.(q.id,Sector)],aes(x=as.Date(q.id),y=N))+geom_line()+facet_wrap(~Sector,ncol=5,scale='free_x')+theme_bw()+geom_smooth(method='loess')+xlab('Years')+ylab('Forecasts')
@

<<fig-stocks-brok,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Number of stocks per broker per quarter',fig.lp='num-s-b'>>=
ggplot(num.f.dt[,.N,by=.(q.id,Sector,Broker)][,mean(N),by=.(q.id,Sector)],aes(x=as.Date(q.id),y=V1))+geom_line()+facet_wrap(~Sector,ncol=5,scale='free_x')+theme_bw()+geom_smooth(method='loess')+xlab('Years')+ylab('Forecasts')
@

<<fig-brok-stocks,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Number of brokers per stock',fig.lp='num-b-s'>>=
ggplot(num.f.dt[,.N,by=.(q.id,Sector,Stock)][,mean(N),by=.(q.id,Sector)],aes(x=as.Date(q.id),y=V1))+geom_line()+facet_wrap(~Sector,ncol=5,scale='free_x')+theme_bw()+geom_smooth(method='loess')+xlab('Years')+ylab('Forecasts')
@
...
<<chp3-desc-rank,echo=F,results='asis'>>=
tab=1
